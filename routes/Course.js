...
router.post(
    ...
    fetchuser,
    [
        body('name', 'Course name must be at least 3 characters...').isLength({ min: 3 }),
        body('description', 'Enter a valid description').isLength({ min: 10 }),
        body('image', 'Enter a valid image URL').isLength({ min: 10 }),
        body('category', 'Enter a valid category').isLength({ min: 6 }),
        body('duration', 'Enter a valid duration').isNumeric(),
    ...
    async (req, res) => {
        try {
            const { name, description, image, category, duration, price, language, rating, numReviews, enrolledStudents } = req.body;
